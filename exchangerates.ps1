...
try {
	...
	"================================"
	$exchangeRates = ListExchangeRates $currency
	$exchangeRates | Format-Table -Property Rate, Currency, Inverse, Date
	
	#Export exchange rates to CSV
	$exchangeRates | Export-Csv -Path "ExchangeRates.csv" -NoTypeInformation
